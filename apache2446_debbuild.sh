...
_build_deb(){
    ...
    cp -a --parents ${apache_location} ${buildroot}
    cp -a --parents /etc/logrotate.d/apache-logs ${buildroot}
    cp -a --parents /etc/logrotate.d/apache-wwwlogs ${buildroot}
...
Architecture: amd64
Eseential: no
Depends: python-dev,libexpat1-dev,zlib1g-dev
Maintainer: Jerry Wang[1976883731@qq.com]
Description: Apache httpd server build by hws
...
EOF
    ...
    dpkg-deb -b ${buildroot} httpd-2.4.46-linux-amd64.deb
}
