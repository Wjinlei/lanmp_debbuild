...
_build_deb(){
    ...
    # Copy files
    cp -a --parents ${php71_location} ${buildroot}
    cp -a --parents /etc/init.d/php71 ${buildroot}
    ...
    # Fix libreadline
    ln -s libreadline.so.7 libreadline.so ${buildroot}${php71_location}/lib
    ...
    # Fix libtinfo
    ln -s libtinfo.so.5 libtinfo.so ${buildroot}${php71_location}/lib
...
Description: php7.1.33 build by hws
...
update-rc.d -f php71 defaults >/dev/null 2>&1
[ $? -ne 0 ] && echo "[ERROR]: update-rc.d -f php71 defaults"
/etc/init.d/php71 start
exit 0
EOF
...
/etc/init.d/php71 stop > /dev/null 2>&1
update-rc.d -f php71 remove >/dev/null 2>&1
exit 0
EOF
...
EOF
    ...
    cd /tmp
    dpkg-deb -b ${buildroot} php-7.1.33-linux-amd64.deb
}
